...
function drag() {
  ...
  function beforestart(id, container, point, that, args) {
    ...
    if (!customEvent(new DragEvent$1(drag, "beforestart", s, id, active, p[0], p[1], 0, 0, sublisteners), function() {
    ...
    return function gesture(type) {
      ...
      customEvent(new DragEvent$1(drag, type, s, id, n, p[0] + dx, p[1] + dy, p[0] - p0[0], p[1] - p0[1], sublisteners), sublisteners.apply, sublisteners, [type, that, args]);
    };
  }
...
var VisConnectUtil = /** @class */ (function () {
    ...
    VisConnectUtil.lasso = function () {
        ...
        var d3l = d3lasso()
            .on('draw', function () {
            var evtData = { detail: { event: d3.event.selection, collaboratorId: d3.event.collaboratorId } };
            var event = new CustomEvent('draw-message', evtData);
            document.body.dispatchEvent(event);
            data.onDraw();
        });
        document.body.addEventListener('draw-message', function (e) {
        });
...
function wrapFct(wrap, inner, fctName) {
    wrap[fctName] = function () {
        inner[fctName].apply(inner, arguments);
        return wrap;
    };
}
