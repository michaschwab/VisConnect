...
(function() {
  ...
  drag.on('dragstart', function() {
    console.log(d3.event);
    active_line[d3.event.sourceEvent.participantId] = {
      points: [],
      color: active_color
    };
    drawing_data.lines.push(active_line[d3.event.sourceEvent.participantId]);
    return redraw(active_line[d3.event.sourceEvent.participantId]);
  });

  drag.on('drag', function() {
    active_line[d3.event.sourceEvent.participantId].points.push(d3.mouse(this));
    return redraw(active_line[d3.event.sourceEvent.participantId]);
  });

  drag.on('dragend', function() {
    if (active_line[d3.event.sourceEvent.participantId].points.length === 0) {
      drawing_data.lines.pop();
    }
    active_line[d3.event.sourceEvent.participantId] = null;
